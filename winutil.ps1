...
if (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    $script = if ($MyInvocation.MyCommand.Path) { "-File $MyInvocation.MyCommand.Path" } else { "-Command $($MyInvocation.Line)" }
    $powershellcmd = if (Get-Command pwsh -ErrorAction SilentlyContinue) { "pwsh.exe" } else { "powershell.exe" }
    $processCmd = if (Get-Command wt.exe -ErrorAction SilentlyContinue) { "wt.exe" } else { $powershellcmd }

    Start-Process -FilePath $processCmd -ArgumentList "-ExecutionPolicy Bypass -NoProfile $script" -Verb RunAs
